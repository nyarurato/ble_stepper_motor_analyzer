...
class Chart:
    ...
    def add_point(self, t_secs: float, y: float):
        ...
        if i == 0:
            ...
            # Initialize the new chunk data with the last
            # point of the previous chunk.
            # if len(self.chunks_data) > 1:
            #     prev_chunk_data = self.chunks_data[-2]
            # else:
            #     # This is the first point ever, no previous
            #     # point,
            #     prev_chunk_data = current_chunk_data

            # last_point =  self.current_chunk_data[-1]

            # #self.current_chunk_data = np.empty((CHUNK_SIZE+1, 2))
            # self.current_chunk_data[0] = last_point
            ...
            # if len(self.curves) == MAX_CHUNKS:
            #    oldest_curve = self.curves[0]

        # If all chunks exists, delete a point from the
        # oldest chunk to compenstate for the point we are adding
